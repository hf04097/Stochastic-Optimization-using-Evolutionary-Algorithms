...
from Mona_lisa import EvolutionaryArt
...
class EA:
    ...
    def runOnce(self, popSize, numGen, mutRate, numOffSpring):
        ...
        for gen in range(numGen):
            ...
            genWiseFitness.append(self.genFitness())
            self.problem.housework(gen, self.generation, genWiseFitness[-1])
...
problem = EvolutionaryArt("mona_image2.jpg")
# problem = KnapsackEA("data/f2_l-d_kp_20_878")

selobj = Selection(problem)

eaobject = EA(problem, selobj.BinaryTournamentSelection, selobj.TruncationSelection)
